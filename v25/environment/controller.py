...
from environment.settings import MAX_EPISODES_V25, SINGLE_EPISODE_LENGTH_V25
...
class ControllerPPOIdealAD:
    def run_c2(self, agent=None, timestamp=None):
        ...
        try:
            ...
            print("\n==============================\nStart Training\n==============================")
            np.random.seed(42)
            training_agent = agent if agent is not None else AgentPPOIdealAD()
    ...
    def loop_episodes(self, agent, timestamp):
        ...
        run_info = f"p{get_prototype()}-{MAX_EPISODES_V25}e-{SINGLE_EPISODE_LENGTH_V25}s"
        ...
        eps_iter = range(1, MAX_EPISODES_V25 + 1) if DEBUG_PRINTING else tqdm(range(1, MAX_EPISODES_V25 + 1))
        for episode in eps_iter:
            ...
            while not is_rw_done():
                ...
                # REWARD Logic
                if is_simulation() and sim_encryption_progress >= SINGLE_EPISODE_LENGTH_V25:
        ...
        all_end = time()
        print(f"steps total {num_total_steps} avg {num_total_steps / MAX_EPISODES_V25}")
        print("==============================")
        print("Generating plots...")
        results_plots_file = plot_average_results(all_summed_rewards, all_avg_rewards, all_num_steps, MAX_EPISODES_V25,
                                                  description)
